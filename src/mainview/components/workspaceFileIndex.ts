...
export function loadWorkspaceIndex(bridge: SmokeBridge, cwd: string): Promise<MentionItem[]> {
...
async function crawl(bridge: SmokeBridge, cwd: string): Promise<MentionItem[]> {
  if (!bridge.isAvailable()) return [];
  const results: MentionItem[] = [];
  const queue: { absolutePath: string; depth: number }[] = [{ absolutePath: cwd, depth: 0 }];
...
export function filterWorkspaceIndex(index: MentionItem[], query: string): MentionItem[] {
